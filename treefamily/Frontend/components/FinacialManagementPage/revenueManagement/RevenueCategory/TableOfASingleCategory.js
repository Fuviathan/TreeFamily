...
export default function TableOfASingleCategory({ id }) {
  const { data, error } = useSWR(`http://localhost:8080/revenue-detail/get-by-id/${id}`)
  console.log(data)
  if (!data) {
    return (
        ...
        <div className="shadow ring-1 ring-black ring-opacity-5 md:rounded-lg ">
          <table className="min-w-full divide-y divide-gray-300 ">
            ...
            <tbody className="bg-white divide-y divide-gray-200 ">
              {/* <For
                each={data}
                render={(item, index) => <ItemInCategory item={item} index={index} />}
              /> */}
            </tbody>
          </table>
        </div>
