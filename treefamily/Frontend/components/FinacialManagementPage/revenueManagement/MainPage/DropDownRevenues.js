...
export default function DropDownRevenues({ item }) {
  ...
  const removeEvent = async (number) => {
    const endpoint = `http://localhost:8080/revenue-management/delete/${number}`;
    const options = {
      method: "DELETE",
      redirect: "follow",
    };
  ...
  return (
    <>
          ...
          <Menu.Items className="absolute right-0 z-20 w-56 mt-2 origin-top-right bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none ">
            <div className="flex flex-col py-1">
              ...
              <Menu.Item>
                {({ active }) => (
                  <div
                    onClick={(e) => { removeEvent(item.id), setShowDeleteModal(true); e.stopPropagation() }}
                    className={classNames(
                      active ? "bg-gray-300 text-gray-900" : "text-gray-700",
                      "block px-4 py-2 text-base"
                    )}
      ...
      <If isTrue={showUpdateModal}>
        <UpdateRevenue isVisible={showUpdateModal} onClose={() => setShowUpdateModal(false)} item={item} />
      </If>
    </>
  );
}
