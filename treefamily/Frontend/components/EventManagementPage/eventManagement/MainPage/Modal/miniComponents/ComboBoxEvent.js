...
export default function ComboBoxEvent(props) {
  ...
  const [selected, setSelected] = useState(event[0]);

  const filteredevent =
    query === ""
      ? event
      : event.filter((nameEvent) =>
          removeVietnameseAccent(
            nameEvent.expenseName.toLowerCase().replace(/\s+/g, "")
          ).includes(
  ...
  const handleSelectedChange = (value) => {
    setSelected(value);
    const expenseManager = event
      .filter((data) => data.expenseName === value)
      .map((obj) => obj.expenseManager);
    props.getOrganizer(expenseManager);
          ...
          <div className="relative mt-1">
            <div>
              <Combobox.Input
                ...
                // displayValue={(nameEvent) => nameEvent.expenseName}
